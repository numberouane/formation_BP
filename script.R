...
df <- readr::read_csv2("individu_reg.csv",
                       col_names = c("region", "aemm", "aged", "anai","catl","cs1",
...
ggplot(df2[as.numeric(df2$aged) > 50,c(3,4)], aes(
  ...
  y = ..density.., fill = factor(decennie_a_partir_annee(as.numeric(aemm)))), alpha = 0.2) + geom_histogram(position = "dodge") 
...
str(df2)
df2[,nrow(df2)-1] <- factor(df2[,ncol(df2)-1])
...
fonction_de_stat_agregee(df  %>% filter(sexe == "2") 
                         %>% mutate(aged = as.numeric(aged)) %>% pull(aged))
fonction_de_stat_agregee(df2 %>% filter(sexe == "Femme") 
                         %>% mutate(aged = as.numeric(aged)) %>% pull(aged))
fonction_de_stat_agregee(df2 %>% filter(sexe == "2" & couple == "2") 
                         %>% mutate(aged = as.numeric(aged)) %>% pull(aged))
fonction_de_stat_agregee(df2 %>% filter(sexe == "Femme" & couple == "2") 
                         %>% mutate(aged = as.numeric(aged)) %>% pull(aged))
